...
class Connect4Game(QMainWindow):
    ...
    def check_win(self):
        ...
        # Diagonal Check (\)
        # I made an edit to this due to a bug I found (check project_report pictures)
    ...
    # Level 2 – Greedy heuristic with one-ply lookahead
    def level_medium(self):

        # Finding all valid columns, this is true when the top cell is 0
        available_cols =[c for c in range(COLS) if self.board[0][c] == 0]
        
        # Move simulation (Simulates each valid move and scores the resulting board)
        if available_cols:
            def dropping(board, col, player):
                ...
                for r in reversed(range(ROWS)):
                    if temp_board[r][col] == 0:
                        ...
                        return temp_board
                    # Column full, return simulated board
                return temp_board
                
            # Simple Herustic, 4 in a row
            def Simple_Heuristic(window, ai):
                ...
                # Reward patterns
                 #  If AI gets four in a row (take immediate wins, but cannot plan multiple moves ahead)
                if window.count(ai) == 4:
                    score += 500

                  # If AI gets three-in-a-row,
                elif window.count(ai) == 3 and window.count(0) == 1:
                    score += 6
                
                # If AI gets two-in-a-row,
                elif window.count(ai) == 2 and window.count(0) == 2:
                    score += 1
                
                # Opponent gets 3 in a row (therefore block obvious danger)
            ...
            def score_board(b, ai = 2):
                # Bonus for playing the center column (4th coulmn)
                score = 0
                center = COLS // 2
                center_values = [b[r][center] for r in range(ROWS)]
                score += center_values.count(ai) * 3 

                # Horizontal Movement
                for r in range(ROWS):
                    row = list(b[r])
                    for c in range(COLS - 3):
                        score += Simple_Heuristic(row[c:c+4],ai)
                
                # Vertical Movement
                for c in range(COLS):
                    col_values = [b[r][c] for r in range(ROWS)]
                    for r in range(ROWS - 3):
                        score += Simple_Heuristic(col_values[r:r+4], ai)
            ...
            # Algorithm used: one-ply greedy search with a hand-crafted heuristic.
            # We assume the AI cannot think of future move ahead of time
            ...
            for col in available_cols:
                ...
                simulations = dropping(self.board, col, 2)
                # Evaluate the board
                score = score_board(simulations, ai = 2)

                # Finding the most optimal move (as score)
                if score > best_score:
                    best_score = score
                    chosen_col = col
            ...
            for r in reversed(range(ROWS)):
                if self.board[r][chosen_col] == 0:
                    ...
                    self.buttons[r][chosen_col].setStyleSheet(
                        f"background-color: yellow; border-radius: {disks_size/2}px;"
                    )
                    ...
                    if self.check_win():
                        self.game_over = True
                        print(f"Player {self.current_player} Wins, Darn!")
                        return
